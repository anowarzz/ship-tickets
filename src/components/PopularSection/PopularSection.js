      ...
      <div className="grid  sm:grid-cols-2 lg:grid-cols-3 mx-auto">
        {ships.length > 0 &&
          ships.map((ship) => {
            return (
              <Link
                className="my-8 px-8 hover:text-teal-600 mx-auto"
                to={`/shipsDetail/${ship?.id}`}
                key={ship?.id}
              >
                ...
                <img
                  src={ship?.image}
                  alt=""
                  className="w-64 h-48 rounded hover:border-blue-500 border-2 border-transparent p-2 "
                />
              </Link>
            );
          })}
